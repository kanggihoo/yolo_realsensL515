...
from bboxes.srv import ImageFirstPickBox
...
class ImagePublisher(Node):
    def __init__(self):
        super().__init__(node_name = 'image_firstpick')
        self.server = self.create_service(ImageFirstPickBox, 'first_picks', self.callback_service)
    ...
    def callback_service(self, request , response):
        ...
        response.color_image = self.br.cv2_to_imgmsg(image)
        if depth_rect_generated is True:
            response.depth_image= self.br.cv2_to_imgmsg(depth_rect)
        else:
            response.depth_image = Image()
        response.success  = depth_rect_generated

        response.x = first_pick['x']
        response.y = first_pick['y']
        response.z = first_pick['z']
        response.center = (first_pick['center'][0] , first_pick['center'][1])
        response.angle = round(first_pick['angle'],1)
        response.class_id = first_pick['label']
        self.get_logger().info("send data : {}".format(first_pick))
        return response
